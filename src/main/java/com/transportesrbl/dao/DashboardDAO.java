...
public class DashboardDAO {
    ...
    // SQL Corregido: La base es ASIGNACION_PAQUETE para que no se pierda ningún producto
    String sql = "SELECT " +
        "COUNT(*) FILTER (WHERE COALESCE(h.Estado, 'Pendiente') = 'En reparto') as activas, " +
        "COUNT(*) FILTER (WHERE COALESCE(h.Estado, 'Pendiente') = 'Completado') as completas, " +
    ...
    // Consulta separada para camiones por claridad
    ...
    public List<Entrega> obtenerEntregasRecientes() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
             ...
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                lista.add(new Entrega(rs.getInt("Id_Asig_Paquete"), rs.getString("Descripcion"), rs.getString("Dir_Entrega"), rs.getString("Estado")));
            }
    ...
    public List<Camion> obtenerEstadoFlota() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
             ...
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                lista.add(new Camion(rs.getString("modelo_camion"), rs.getDouble("capacidad_m3"), rs.getString("estado")));
            }
        } catch (SQLException e) { e.printStackTrace(); }
        return lista;
    }
